...
from chromadb.lmi.index.model import ModelParameters
...
class BuildConfiguration:
    ...
    def _validate(
        ...
        assert (
            type(clustering_algorithms) == list
            and type(epochs) == list
            and type(model_types) == list
            and type(lrs) == list
        ) or (
            callable(clustering_algorithms)
            and type(epochs) == int
            and type(model_types) == str
            and type(lrs) == float
        ), "clustering_algorithms, epochs, model_types, and lrs must be lists or single values"

        for arg in [clustering_algorithms, epochs, model_types, lrs]:
            if type(arg) == list:
